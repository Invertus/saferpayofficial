...
class SettingsTranslationService
{
    ...
    private function getGeneralSettingsTranslations()
    {
        return [
            ...
            'configNameDescription' => html_entity_decode($this->module->l('Payment Page Configurations are created in the Saferpay Backoffice (Settings > Payment Page Configuration). Leave empty to use the Saferpay default styling.', self::FILE_NAME), ENT_QUOTES, 'UTF-8'),
            'selectConfigName' => $this->module->l('Select a configuration', self::FILE_NAME),
            'noConfigNameOption' => $this->module->l('None - Saferpay default styling', self::FILE_NAME),
            'configNameNotInAccount' => $this->module->l('%s (not found in this account)', self::FILE_NAME),
            'savedConfigNameMissing' => $this->module->l('The saved configuration is not in this Saferpay account. Keep it to leave the stored value untouched, or pick another one.', self::FILE_NAME),
            'enterCredentialsToLoadConfigNames' => $this->module->l('Enter your API username and password first to load available configurations.', self::FILE_NAME),
            'noConfigNamesInAccount' => $this->module->l('This Saferpay account has no payment page configurations.', self::FILE_NAME),
            'failedToFetchConfigNames' => $this->module->l('Failed to load payment page configurations', self::FILE_NAME),
            'loadingConfigNames' => $this->module->l('Loading configurations...', self::FILE_NAME),
